...
parser.add_argument('--k', dest='k', type=int,
					default = 16,
                    help='k cosine nearest neighbourd')	#!!!!!!!!!!!!!!!!!!
...
class Solver:
	...
	def __init__(self, params):
		...
		params['inp_dim'] = self.data_handler.getDataShape()[1]		# inp_dim = 1000
		logging.info("="*41)
	...
	def train(self, params):
		...
		for iteration in range(num_epochs):		# for each epoch
			...
			num_batches = self.data_handler.getNumberOfBatches(batch_size) 	# num_batches = number of iterations
			epoch_losses = np.zeros(5) # rl, asl, psl, total !!!!!!!!!!!!!!!!!!
			for batch_idx in range(num_batches):	# for each iteration
				...
				batch_y = Variable(torch.from_numpy(batch_y), requires_grad=False).type(dtype)	#dtype = torch.FloatTensor
